...
class Car:
	...
	print("what is fucking settings:", settings_f)
        if os.path.exists(settings_f):
            ...
            self.halfR = setting[1][1] + setting[1][0] * 50
            self.fullR = setting[1][1] + setting[1][0] * 100
            self.left_duty = 50
            self.right_duty = 50
    ...
    def set_speed(self, left_speed=90, right_speed=90):
        def get_left_duty(left_speed):
            ...
            if left_speed < 0:
                self.left_duty = -((-left_speed - self.halfL)
                                   * 50 / (self.fullL - self.halfL) + 50)
        ...
        def get_right_duty(right_speed):
            ...
            if right_speed < 0:
                self.right_duty = -((-right_speed - self.halfR)
                                    * 50 / (self.fullR - self.halfR) + 50)
    ...
    def test(self):
        ...
        '''
        print("Please input %s, %s, %s ,%s and %s to control" %
              ('w', 'a', 's', 'd', 'q'))

...
class Encoder:
    ...
    def command(self, measure=0):
        if self.measure == 0 and measure == 1:
            ...
            GPIO.add_event_detect(self.INleft, GPIO.RISING,
                                  callback=self.callback_left)
            GPIO.add_event_detect(self.INright, GPIO.RISING,
    ...
    def kalman_filter(self, speed=0, left_or_right="left"):
        ...
        p_mid = self.p_lastleft + \
            self.Q if left_or_right == "left" else self.p_lastright + self.Q
    ...
    def func_speed(self):
        if self.measure == 1:
            ...
            speed_right = 1.052 * (self.count_right -
                                   self.lastcount_right) * 2.5
    ...
    def test(self):
        print("Please input %s to start, %s to stop, %s, %s to get distance and speed" % (
            'q', 'w', 'e', 'r'))
                ...
                if len(c) != 0:
                    if c in ['q', 'w', 'e', 'r']:
                        if c == "q":
                            ...
                            print("left_distance: %.2f cm, right_distance: %.2f cm" % (
                            ...
                            while True:
                                ...
                                time.sleep(0.4)
                                print(
                                    "left_speed: %.2f cm/s, right_speed: %.2f cm/s" % (left_speed, right_speed))
...
if __name__ == '__main__':
    ...
    # cm/s, because of Kalman filter, the speed will be stable after 3 or 4 seconds.
    left, right = encoder.get_speed()
    # the running distance after encoder.command(1)
    left, right = encoder.get_distance()
